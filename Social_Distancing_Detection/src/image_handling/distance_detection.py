...
class DistanceDetector:
    ...
    def __init__(self, image_file = None, image = [], pixel_tolerance = 100, adj_height = 750):
        self.image_file = image_file
        self.pixel_tolerance = pixel_tolerance
        self.adj_height = adj_height
        if len(image) > 0:
            self.image = imutils.resize(image, height=self.adj_height)
        else:
        ...
        for face_combo in self.face_distances:
            ...
            if distance < self.pixel_tolerance:
                ...
                cv2.putText(self.image, "Faces in this image not following social distancing",
                            (50, self.adj_height - 50), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 255, 0), 2)
